...
exports.registerUser = async (req, res, next) => {
    try {
        ...
        await newCart.save();
        await emailService.sendWelcomeEmail({ body: { to: email, name: username } }, res, next);
        res.status(201).json({ message: 'User registered successfully', userId: savedUser._id });
        } catch (error) {
            return next(res.status(500).json({ message: 'Error registering user', error: error.message }));
        }
    };
